...
def generate_animation(
    ...
    """
    生成一个关节从起始角度旋转到终止角度的动画 GIF。
    joint_name: 关节名称，与 build_demo_pose 中的一致
    start_angle / end_angle: 绕该关节默认轴的旋转角度 (弧度)
    num_frames: 帧数
    """
    # 记录关节在 body_pose 中的起始索引
    joint_names = {
        "left_hip": 1,
        "right_hip": 2,
        "left_knee": 4,
    ...
    if joint_name not in joint_names:
        raise ValueError(f"未知关节: {joint_name}，可选: {list(joint_names.keys())}")

    start_idx = (joint_names[joint_name] - 1) * 3

    # 获取默认姿态（保持其他关节不变）
    global_orient, base_body_pose = build_demo_pose(device, dtype)
    # 动画中，让该关节从 start_angle 变化到 end_angle
    ...
    print(f"生成动画帧：{joint_name} {start_angle:.2f} → {end_angle:.2f}")

    for i, angle in enumerate(angles):
        body_pose = base_body_pose.clone()
        # 根据关节设定轴角（这里简单沿 Y 轴旋转，可根据需要调整）
        body_pose[0, start_idx:start_idx + 3] = torch.tensor(
        ...
        joints_np = to_numpy(data["J_transformed"][0])
        ...
        draw_mesh(
            ...
            faces,
            joints=joints_np,
            title=f"Frame {i+1}/{num_frames}  angle={angle:.2f} rad",
        )
...
def main(args):
    ...
    if args.animate:
        generate_animation(
            ...
            dtype,
            out_dir,
            joint_name=args.animate_joint,
            start_angle=args.animate_start,
            end_angle=args.animate_end,
...
if __name__ == "__main__":
    ...
    # 动画参数
    parser.add_argument("--animate", action="store_true", help="是否生成姿态动画")
    parser.add_argument("--animate-joint", type=str, default="left_elbow", help="动画关节名")
    parser.add_argument("--animate-start", type=float, default=0.0, help="起始角度 (弧度)")
    parser.add_argument("--animate-end", type=float, default=-0.9, help="终止角度 (弧度)")
    parser.add_argument("--animate-frames", type=int, default=24, help="动画帧数")
